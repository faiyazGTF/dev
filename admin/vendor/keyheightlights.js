$(document).ready(function(){
	$(document).on('submit','#addkeyhightlights', function(e){
		e.preventDefault(0);
		var formData = new FormData(this);
		
		$.ajax({
			url : '__ajax.php?action=addkeyhightlights',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
...
$(document).on('click','.editkeyheightlihgts',function(){
	var dataid=$(this).attr('dataid');


	$.ajax({
		url : '__ajax.php?action=editkeyheightlihgts',
		type : 'POST',
		data : {'dataid' : dataid},
		success : function(resp){
	...
	$(document).on('submit','#updateothermicrodata', function(e){
		...
		$.ajax({
			url : '__ajax.php?action=updatekeyhightlights',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
