<?php 
	...
	if(empty($_FILES['feature_image']['name'])){
		$validator['feature_image']="This fields is required";
		$error=1;
	}
	...
	$wp_no = $_POST['project_whatapp_no'];
		...
		if(isset($_FILES['feature_image']) && !empty($_FILES['feature_image'])){
			...
			$brochure_date = date('Y-m-d-h-i-s');
			$new_brochure = "feature-image-$brochure_date.$image_ext";
			
			$meta_path = "../../uploads/microsite/feature-image/$new_brochure";
			if(move_uploaded_file($meta_logo_tmp_name, $meta_path)){
				$feature_image = $new_brochure;
			}
		}
		 

		$starting_price=$_POST['starting_price'];
		$query = mysqli_query($conn, "INSERT INTO micro_site (platter_id,agent_rera,feature_image,page_url,developer_id, cat_id, type_id, status_id, state_id, city_id, locality_id, title, keyword, description, project_brochure, name, address, payment_plan, rera_no, project_ivr_no, whatapp_no,starting_price) VALUES ('$platter_page','$agent_rera','$feature_image','$page_url','$dev_id', '$cat_id', '$type', '$status', '$state', '$city', '$location', '$meta_title', '$meta_keyword', '$meta_desc', '$project_image_brochure', '$name', '$address', '$price', '$rera',  '$ivr_call', '$wp_no','$starting_price')");
		if($query==1){
