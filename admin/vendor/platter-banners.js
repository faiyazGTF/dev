$(document).ready(function(){
	$(document).on('submit','#addbanner', function(e){
		...
		$.ajax({
			url : '__platter.php?action=addbanners',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
...
$(document).on('click','.editplatterbanenrs',function(){
	...
	$.ajax({
		url : '__platter.php?action=editplatterbanenrs',
		type : 'POST',
		data : {'dataid' : dataid},
		success : function(resp){
			var result=JSON.parse(resp);
				if(result.status==1){
					...
					$("#upddatefilemobile").after('<img  src="'+ADMIN_URL+record['mobile_banner']+'" class="removdeelemensts" width="100" >');
	...
	$(document).on('submit','#updatebanenrform', function(e){
		...
		$.ajax({
			url : '__platter.php?action=updateplatterbanner',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
...
if(confirm(text) == true){
	...
	$.ajax({
		url : '__platter.php?action=deleteplatterbanner',
		type : 'POST',
		data : {'dataid' : dataid},
		success : function(resp){
