$(document).ready(function(){
	$(document).on('submit','#addfloorplands', function(e){
		...
		$.ajax({
			url : '__ajax.php?action=addmicrogallery',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
...
$(document).on('click','.editothermicrosites',function(){
	...
	$.ajax({
		url : '__ajax.php?action=editgalleryplanimages',
		type : 'POST',
		data : {'dataid' : dataid},
		success : function(resp){
			var result=JSON.parse(resp);
				if(result.status==1){
					...
					$("#imagealtupdate").val(record['imagealt']);
	...
	$(document).on('submit','#updateothermicrodata', function(e){
		...
		$.ajax({
			url : '__ajax.php?action=updaremicrogallery',
			type: "POST",
			data: formData,
			dataType: 'json',
			contentType: false,
