<?php 
	...
	if(mysqli_num_rows($chk_query) > 0 && mysqli_num_rows($chk_list_query) > 0){
		...
		$update_sql = "UPDATE micro_site_location SET image = '$update_img_name', iframe = '$update_iframe_name'  WHERE project_id = '$update_id'";
		...
		if(isset($_POST['iframe']) && $_POST['iframe'] != ''){
			...
			$ins_query = mysqli_multi_query($conn,"INSERT INTO micro_site_location (project_id, image, iframe) VALUES ('$update_id', '$ins_img_name', '$iframe')");
			if($ins_query==1){
				echo "success";
			}else{
				echo "failed";
